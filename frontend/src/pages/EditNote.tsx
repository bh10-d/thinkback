...
import {
  ...
  Space, Alert, Divider, Spin, message,
} from 'antd';
...
export default function EditNote() {
  ...
  useEffect(() => {
    if (!id) return;
    Promise.all([notesApi.getById(id), topicsApi.getAll()])
      .then(([note, t]) => {
        setTopics(t);
        form.setFieldsValue({
          title: note.title,
          questions: note.questions.length > 0 ? note.questions : [''],
  ...
  async function handleSubmit(values: {
    ...
    setSubmitting(true);
    try {
      await notesApi.update(id, { ...values, questions: filled });
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div>
          ...
          <Text type="secondary">Review progress will not be reset</Text>
        </div>
      </div>
      ...
      <Card>
        <Form form={form} layout="vertical" onFinish={handleSubmit}>
          ...
          <Form.Item
            label={<span>Recall Questions <span style={{ color: '#faad14' }}>⭐</span></span>}
            required
          >
          ...
          <Form.Item label="Core Idea (1–3 lines)" name="coreIdea" rules={[{ required: true, message: 'Required' }]}>
            <TextArea rows={3} placeholder="The essence of what you need to remember" />
          </Form.Item>

          <Form.Item label="Common Mistake" name="mistake" rules={[{ required: true, message: 'Required' }]}>
            <Input placeholder="What do people get wrong about this?" />
          </Form.Item>

          <Form.Item label="Example" name="example" rules={[{ required: true, message: 'Required' }]}>
            <TextArea
              rows={5}
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <Button onClick={() => navigate(`/notes/${id}`)} size="large">
              Cancel
            </Button>
          </div>
        </Form>
